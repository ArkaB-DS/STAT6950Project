...
dev.off()
# OTHER SUMMARY PLOTS
subdb19 <- dplyr::select(db19, c(GE2019, PV2019, RQ2019, RL2019, VA2019))
...
db14 <- database %>%
  dplyr::select(matches('country|2014')) # add RGDP, rate of change in GDP from 2018-2019
db14 <- dplyr::select(db14, -GDP2014) # drop GDP2019 
...
Data[, "NFCR2019"] <- sapply(Data[,"NFCR2019"], function (x) gsub('%', '', x))
Data[,"NFCR2019"] <- as.numeric(Data[,"NFCR2019"])
